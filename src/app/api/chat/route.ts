...
export async function POST(req: NextRequest) {
  ...
  const forceBoost = body?.forceBoost === true;
...
(plan === ("free" as any) ? freeLiteModeInstruction(locale) + "\n" : "") +
  ...
  let plusBoostDecision = shouldUsePlusBoost({
    plan,
    usage,
    messages,
    lastUserText: lastTextOriginal,
  ...
  if (
    forceBoost &&
    plan === ("plus" as any) &&
    Number(usage.boostThisMonth || 0) < PLUS_BOOST_LIMIT
  ) {
    plusBoostDecision = { useBoost: true, reason: "frontend-force-boost" };
  }
  ...
  async function callViaGeminiNonStream(): Promise<string> {
    ...
    const text = await callGeminiGenerateContent({
      ...
      promptText: `${instructions}` + "`n`n---`n`n" + `${inputText}`,
