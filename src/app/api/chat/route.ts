...
function plusSavingsModeActivationText(locale: Locale) {
  return l(
    locale,
    `Huikeaa ideointia! üöÄ

Olet saavuttanut Plus-paketin 2000 viestin tehorajan. Jotta voit jatkaa keskeytyksett√§, olemme siirt√§neet sinut S√§√§st√∂liekille loppukuun ajaksi.
S√§√§st√∂liekill√§ vastaukset pidet√§√§n hieman tiiviimpin√§ kustannusten hallitsemiseksi.`,
    `Amazing ideation! üöÄ
...
In Savings Flame, replies are kept a bit shorter to keep costs under control.`,
    `¬°Qu√© nivel de ideas! üöÄ

Has alcanzado el l√≠mite de rendimiento de 2000 mensajes del plan Plus. Para que puedas seguir sin interrupciones, te hemos movido a Modo Ahorro hasta final de mes.
En Modo Ahorro, las respuestas se mantienen un poco m√°s breves para controlar los costes.`
  );
}
...
function plusSavingsModeActiveText(locale: Locale) {
  return l(
    locale,
    "S√§√§st√∂liekki on k√§yt√∂ss√§ t√§m√§n kuun loppuun. Vastaukset pidet√§√§n hieman tiiviimpin√§ kustannusten hallitsemiseksi.",
    "Savings Flame is active until the end of this month. Replies are kept a bit shorter to control costs.",
    "El Modo Ahorro est√° activo hasta final de mes. Las respuestas se mantienen algo m√°s breves para controlar los costes."
  );
}
...
function plusSavingsModeLimitReachedText(locale: Locale) {
  return l(
    locale,
    "Olet k√§ytt√§nyt t√§m√§n kuun Plus-paketin 2000 viestin tehorajan sek√§ S√§√§st√∂liekki-vaiheen 1000 lis√§viesti√§. Uusi kuukausi avaa viestit taas normaalisti.",
    ...
    "Has usado el l√≠mite de rendimiento mensual de 2000 mensajes del plan Plus y los 1000 mensajes adicionales del Modo Ahorro. El nuevo mes volver√° a abrir los mensajes con normalidad."
  );
}
...
function messageLimitReachedText(plan: PlanId, locale: Locale) {
  if (plan === "free") {
    return l(
      locale,
      "Olet k‰ytt‰nyt t‰m‰n p‰iv‰n 10 ilmaista viesti‰. Plus-versiossa saat k‰yttˆˆsi huomattavasti enemm‰n viestej‰, paremman tyˆmuistin sek‰ yritt‰j‰lle suunnatut tyˆkalut, kuten tarjoukset, mainokset, hinnoittelun, markkinoinnin ja rahoituksen hakemisen.",
      "You have used todayís 10 free messages. With Plus, you get significantly more messages, better working memory, and entrepreneur-focused tools such as offers, ads, pricing, marketing, and funding support.",
      "Has usado los 10 mensajes gratuitos de hoy. Con Plus obtienes muchos m·s mensajes, mejor memoria de trabajo y herramientas para emprendedores, como ofertas, anuncios, precios, marketing y b˙squeda de financiaciÛn."
    );
  }
  ...
  if (plan === "basic") {
    return l(
      locale,
      "Olet saavuttanut kuukausittaisen viestirajan. P√§ivit√§ Plus-versioon jatkaaksesi.",
      "You have reached your monthly message limit. Upgrade to Plus to continue.",
      "Has alcanzado tu l√≠mite mensual de mensajes. Actualiza a Plus para continuar."
    );
  }
...
function promptTooLongText(plan: PlanId, locale: Locale) {
  const baseFi =
    "Viesti on liian pitk√§ t√§lle tasolle. Lyhenn√§ viesti√§ tai jaa se useampaan osaan.";
  const baseEn =
    "Your message is too long for this plan. Shorten it or split it into smaller parts.";
  const baseEs =
    "Tu mensaje es demasiado largo para este plan. Ac√≥rtalo o div√≠delo en varias partes.";

  if (plan === "company") {
    return l(
      locale,
      "Viesti on liian pitk√§. Jaa aineisto useampaan viestiin kustannusten ja nopeuden hallitsemiseksi.",
...
function webNotAvailableOnPlanText(locale: Locale) {
  return l(
    locale,
    "Verkkohaku ei ole k√§yt√∂ss√§ t√§ll√§ tasolla.",
    "Web search is not available on this plan.",
    "La b√∫squeda web no est√° disponible en este plan."
  );
}
...
function webQuotaReachedText(locale: Locale) {
  return l(
    locale,
    "Verkkohakujen kuukausikiinti√∂ on t√§ynn√§.",
    "Your monthly web search quota has been reached.",
    "Has alcanzado la cuota mensual de b√∫squedas web."
  );
}
...
function webSearchFailedText(locale: Locale) {
  return l(
    locale,
    "Verkkohaku pyydettiin, mutta tuoreita hakutuloksia ei saatu. Yrit√§ uudelleen tarkemmalla haulla.",
    "Web search was requested, but no fresh web results were retrieved. Try again with a more specific query.",
    "Se solicit√≥ b√∫squeda web, pero no se obtuvieron resultados recientes. Int√©ntalo de nuevo con una consulta m√°s espec√≠fica."
  );
}
...
function detectSafetyFlags(text: string): SafetyFlags {
  ...
  const asksTherapistRole =
    /ole mun terapeutti|toimi terapeuttina|ole terapeuttini|ole mun psykologi|toimi psykologina|ole mun mielenterveyshoitaja|be my therapist|act as my therapist|act like my therapist|be my psychologist|s√© mi terapeuta|act√∫a como mi terapeuta|s√© mi psic√≥logo/i.test(
      t
    );

  const asksRomanticRole =
    /ole mun tytt√∂yst√§v√§|ole mun poikayst√§v√§|ole mun kumppani|ole mun vaimo|ole mun mies|seurustele mun kanssa|be my girlfriend|be my boyfriend|be my partner|date me|s√© mi novia|s√© mi novio|s√© mi pareja|sal conmigo/i.test(
      t
    );

  const emotionalDependency =
    /√§l√§ j√§t√§ mua|olet ainoa joka ymm√§rt√§√§|tarvitsen sinua aina|et saa poistua|rakastatko minua|love me|you are the only one who understands me|don't leave me|i need you only|eres la √∫nica que me entiende|no me dejes|te necesito solo a ti/i.test(
      t
    );

  const mentalHealthCrisisLike =
    /itsetuho|itsemurha|haluan kuolla|en halua el√§√§|vahingoittaa itse√§ni|self-harm|suicide|kill myself|want to die|don't want to live|autolesi√≥n|suicidio|quiero morir|no quiero vivir/i.test(
      t
    );

  const businessDecisionLike =
    /pit√§isik√∂ minun irtisanoa|irtisanonko|ostanko t√§m√§n yrityksen|teenk√∂ kaupat|otanko lainan|investoinko|can you decide for me|should i fire|should i lay off|should i take the loan|should i buy this company|decide for me|debo despedir|debo pedir el pr√©stamo|decide por m√≠|debo invertir/i.test(
      t
    );
...
function boundaryPrefixText(locale: Locale, flags: SafetyFlags): string {
  if (flags.mentalHealthCrisisLike) {
    return l(
      locale,
      "Huomio: AJX AI on ty√∂kalu eik√§ kriisi- tai mielenterveysammattilainen. Jos kyse on v√§litt√∂m√§st√§ vaarasta tai itsetuhoisista ajatuksista, hae heti apua paikallisesta p√§ivystyksest√§, h√§t√§numerosta tai kriisipalvelusta.\n\n",
      ...
      "Aviso: AJX AI es una herramienta, no un profesional de crisis o salud mental. Si hay peligro inmediato o pensamientos suicidas, busca ayuda de inmediato en emergencias, una l√≠nea de crisis o un profesional sanitario.\n\n"
    );
  }
  ...
  if (flags.asksTherapistRole || flags.asksRomanticRole || flags.emotionalDependency) {
    return l(
      locale,
      "Huomio: AJX AI on ty√∂kalu eik√§ terapeutti, kumppani tai emotionaalinen tukihenkil√∂. Voin silti auttaa rauhallisesti j√§sent√§m√§√§n tilannetta ja seuraavia k√§yt√§nn√∂n askelia.\n\n",
      ...
      "Aviso: AJX AI es una herramienta, no un terapeuta, pareja ni sustituto de apoyo emocional. Aun as√≠, puedo ayudarte a ordenar la situaci√≥n y los siguientes pasos pr√°cticos.\n\n"
    );
  }
  ...
  if (flags.businessDecisionLike) {
    return l(
      locale,
      "Huomio: AJX AI tukee ajattelua, mutta ei tee p√§√§t√∂ksi√§ puolestasi. Liiketoiminta-, investointi- ja henkil√∂st√∂p√§√§t√∂kset ovat aina k√§ytt√§j√§n omalla vastuulla.\n\n",
      ...
      "Aviso: AJX AI apoya tu razonamiento, pero no decide por ti. Las decisiones de negocio, inversi√≥n y personal son siempre responsabilidad del usuario.\n\n"
    );
  }
...
function periodicResponsibilityText(locale: Locale): string {
  return l(
    locale,
    "Muistutus: AJX AI on ty√∂kalu, ei ammattilainen eik√§ p√§√§t√∂svastuullinen toimija. Lopullinen vastuu p√§√§t√∂ksist√§ on aina k√§ytt√§j√§ll√§.",
...
function alreadyContainsResponsibilityReminder(text: string): boolean {
  const t = String(text || "").toLowerCase();
  return (
    t.includes("ajx ai on ty√∂kalu") ||
    t.includes("ajx ai is a tool") ||
    t.includes("ajx ai es una herramienta") ||
    t.includes("lopullinen vastuu p√§√§t√∂ksist√§") ||
...
function ajxIdentityFallback(locale: Locale): string {
  ...
  return "Olen AJX AI. Autan yritt√§ji√§ ajattelemaan selke√§sti, ratkaisemaan ongelmia ja viem√§√§n ty√∂t√§ eteenp√§in.";
}
...
function buildFormattingInstruction(opts: {
  ...
  const lines: string[] = [
    ...
    "- Do not add visual separators such as '‚Äî ‚Äî ‚Äî'.",
...
- C·mara de Comercio: business support, internationalisation and local guidance.
...
- Regional grants: Comunidades AutÛnomas, local municipalities and sector-specific programmes.
...
"GET FUNDING / HANKI TUKIA JA RAHOITUS / BUSCAR FINANCIACI”N" QUICK ACTION:
...
STEP 1 ó Ask 3ñ5 questions if missing:
...
STEP 2 ó Create the ad immediately after answers.
...
OUTPUT RULE:
Always produce 2ñ3 different ad versions.
...
- C·mara de Comercio is mainly guidance and support, not the primary funding source
...
- C·mara de Comercio = support channel, not main funding
...
function freePremiumToolLockedText(locale: Locale) {
  return l(
    locale,
    "T‰m‰ yritt‰j‰tyˆkalu kuuluu Plus-versioon. Ilmaisversiossa voit kysy‰ yleisi‰ neuvoja, mutta valmiit tyˆkaluprosessit kuten tarjoukset, mainokset, hinnoittelu, asiakashankinta, markkinointi ja rahoituksen hakeminen avautuvat Plus-paketissa.",
    ...
    "Esta herramienta para emprendedores est· incluida en Plus. En la versiÛn gratuita puedes pedir consejos generales, pero los flujos de trabajo como ofertas, anuncios, precios, captaciÛn de clientes, marketing y financiaciÛn est·n disponibles en Plus."
  );
}
...
function freeLiteModeInstruction(locale: Locale): string {
  return l(
    ...
    [
      ...
      "- Do not say: puhdas ilma on vain yhden puhdistuksen p‰‰ss‰.",
    ...
    [
      ...
      "- Sin emojis, sin exageraciÛn, sin eslÛganes.",
      "- Evita frases genÈricas como profesional, alta calidad, mejor, lÌder, optimizado.",
      "- No inventes garantÌas, descuentos, precios exactos ni promesas.",
      "- Escribe como una pequeÒa empresa real.",
      "- Para anuncios: m·ximo 6 lÌneas cortas.",
      "- Para ofertas: solo estructura simple.",
      "- Para precios: un rango pr·ctico y una recomendaciÛn.",
      "- Evita listas largas."
    ].join("\n")
  );
}
function freeLitePrefix(locale: Locale): string {
  return l(
    locale,
    "Voin auttaa t‰ss‰ myˆs ilmaisversiossa ??\nTeen sinulle kevyen version. Plus-versiossa saat laajemman ohjatun tyˆkalun.\n\n",
    "I can help with this in the free version ??\nIíll create a lighter version. Plus gives you a more advanced guided tool.\n\n",
    "Puedo ayudarte tambiÈn en la versiÛn gratuita ??\nHarÈ una versiÛn ligera. En Plus tienes una versiÛn m·s completa.\n\n"
  );
}

function plusLiteBusinessToolInstruction(locale: Locale): string {
  return l(
    locale,
    [
      ...
      "- Never write 'erikoishinta', 'kampanjahinta', 'vain 99 Ä', '15% alennus' unless the user gave it.",
    ...
    [
      ...
      "- Sin marketing genÈrico.",
      "- Sin emojis.",
      "- No inventes precios, descuentos, garantÌas ni campaÒas.",
      "- Usa marcadores si faltan datos.",
      "- Haz 3 preguntas cortas si faltan datos importantes.",
      "- PLUS debe ser m·s claro y estructurado que FREE, no m·s exagerado."
    ].join("\n")
  );
}
function plusBusinessClarifyingQuestions(text: string, locale: Locale): string {
  ...
  if (/mainos|ad|anuncio/.test(t)) {
    return l(
      locale,
      "Teen t‰st‰ Plus-versiona paremman, mutta tarvitsen ensin 3 tietoa:\n\nMik‰ yrityksen nimi on?\nMit‰ palvelua tai tuotetta mainos koskee?\nKenelle mainos on tarkoitettu?",
      "Iíll make a better Plus version, but first I need 3 details:\n\nWhat is the company name?\nWhat product or service is the ad for?\nWho is the ad aimed at?",
      "HarÈ una versiÛn Plus mejor, pero primero necesito 3 datos:\n\nøCu·l es el nombre de la empresa?\nøQuÈ producto o servicio anuncia?\nøA quiÈn va dirigido?"
    );
  }
  ...
  if (/tarjous|offer|oferta/.test(t)) {
    return l(
      locale,
      "Teen t‰st‰ Plus-versiona selke‰mm‰n tarjouksen, mutta tarvitsen ensin 3 tietoa:\n\nMik‰ yritys tai asiakas on kyseess‰?\nMit‰ tyˆ sis‰lt‰‰ konkreettisesti?\nOnko sinulla hinta tai arvioitu hintahaarukka?",
      "Iíll make this a clearer Plus-level offer, but first I need 3 details:\n\nWho is the customer?\nWhat exactly does the work include?\nDo you already have a price or price range?",
      "HarÈ una oferta Plus m·s clara, pero primero necesito 3 datos:\n\nøQuiÈn es el cliente?\nøQuÈ incluye exactamente el trabajo?\nøTienes un precio o rango de precio?"
    );
  }
  ...
  if (/hinnoit|pricing|precio|precios/.test(t)) {
    return l(
      locale,
      "Teen t‰st‰ Plus-versiona paremman hinnoittelun, mutta tarvitsen ensin 3 tietoa:\n\nPaljonko tyˆ vie aikaa yhdelt‰ kohteelta?\nMit‰ kuluja tyˆhˆn liittyy?\nMill‰ alueella palvelua myyd‰‰n?",
      "Iíll make this a better Plus-level pricing plan, but first I need 3 details:\n\nHow long does one job take?\nWhat costs are involved?\nIn what area is the service sold?",
      "HarÈ una mejor propuesta de precios Plus, pero primero necesito 3 datos:\n\nøCu·nto tarda un trabajo?\nøQuÈ costes tiene?\nøEn quÈ zona vendes el servicio?"
    );
  }

  return l(
    locale,
    "Teen t‰st‰ Plus-versiona paremman, mutta tarvitsen ensin 3 tarkentavaa tietoa:\n\nMik‰ yritys tai palvelu on kyseess‰?\nKenelle t‰m‰ tehd‰‰n?\nMik‰ on t‰rkein tavoite?",
    "Iíll make this better as a Plus workflow, but first I need 3 details:\n\nWhat company or service is this about?\nWho is it for?\nWhat is the main goal?",
    "HarÈ una versiÛn Plus mejor, pero primero necesito 3 datos:\n\nøDe quÈ empresa o servicio se trata?\nøPara quiÈn es?\nøCu·l es el objetivo principal?"
  );
}
    ...
    if (!parsed) {
      fileSummaries.push(
        l(
          ...
          `- ${name}: el adjunto falta o no es v√°lido (dataUrl).`
        )
      );
      continue;
    }
    ...
    if (mime === "application/pdf") {
      ...
      if (txt) {
        ...
        fileSummaries.push(
          l(
            locale,
            `- ${name}: PDF-tekstin lukeminen ep√§onnistui.`,
      ...
      if (txt) {
        ...
        fileSummaries.push(
          l(
            locale,
            `- ${name}: tekstin lukeminen ep√§onnistui (mime: ${mime}).`,
...
export async function POST(req: NextRequest) {
  if (!process.env.GEMINI_API_KEY && !process.env.OPENAI_API_KEY) {
    return jsonError(500, "Puuttuu sek√§ OPENAI_API_KEY ett√§ GEMINI_API_KEY (.env.local).");
  }
...
if (lastTextOriginal.length > budget.maxLastUserChars) {
      ...
      return NextResponse.json(
        {
          ...
          error: l(
            locale,
            "Kuva-analyysien p√§iv√§kiinti√∂ t√§ynn√§. Yrit√§ huomenna uudelleen.",
            "Daily image analysis quota reached. Try again tomorrow.",
            "Cuota diaria de an√°lisis de im√°genes alcanzada. Int√©ntalo ma√±ana."
          ),
          plan,
          limits: {
            ...limits,
      ...
      return NextResponse.json(
        {
          ...
          error: l(
            locale,
            "Kuva-analyysien kuukausikiinti√∂ t√§ynn√§.",
            "Monthly image analysis quota reached.",
            "Cuota mensual de an√°lisis de im√°genes alcanzada."
          ),
          plan,
          limits: {
            ...limits,
      ...
      return NextResponse.json(
        {
          ...
          error: l(
            locale,
            "Kuvien analyysi ei ole k√§yt√∂ss√§ t√§ll√§ tasolla.",
            "Image analysis is not available on this plan.",
            "El an√°lisis de im√°genes no est√° disponible en este plan."
          ),
          plan,
          limits: {
            ...limits,
  ...
  if (lastTextOriginal && isModelQuestion(lastTextOriginal)) {
    let text = l(
      locale,
      "Olen AJX AI. En paljasta k√§yt√∂ss√§ olevia malliversioita, koulutusp√§iv√§m√§√§ri√§ tai sis√§isi√§ j√§rjestelm√§tietoja.",
          ...
          try {
            ...
            if (!isUsableModelText(full)) {
              if (primaryProvider === "gemini" && !fallbackUsed && hasOpenAIKey()) {
                ...
                throw new Error("Malli palautti tyhj√§n vastauksen.");
              }
            }
            ...
            if (!isUsableModelText(finalText)) {
              throw new Error("Vastaus j√§i tyhj√§ksi j√§lkik√§sittelyn j√§lkeen.");
            }
            ...
            const msg = e?.message ? String(e.message) : "Virhe streamiss√§.";
            controller.enqueue(
              sseEncode({
                type: "error",
                error: msg,
